...
void SkinnedMeshApp::UpdateImgui(const GameTimer& gt)
{
...
void SkinnedMeshApp::UpdatePerObjectCB(const GameTimer& gt)
{
    ...
    for(UINT i = 0; i < texNames.size(); ++i)
    {
        if(texLib.Contains(texNames[i]) == false)
        {
            ...
            ThrowIfFailed(DirectX::CreateDDSTextureFromFileEx(md3dDevice.Get(), *mUploadBatch.get(),
                          texMap->Filename.c_str(), 0, D3D12_RESOURCE_FLAG_NONE, DDS_LOADER_MIP_AUTOGEN,
...
void SkinnedMeshApp::LoadGeometry()
{
...
bool SkinnedMeshApp::LoadSkinnedModel()
{
    std::vector<M3DLoader::SkinnedVertex> vertices;
    std::vector<UINT> indices32;

    M3DLoader m3dLoader;
    if(!m3dLoader.LoadM3d(mSkinnedModelFilename, vertices, indices32,
       mSkinnedSubsets, mSkinnedMats, mSkinnedInfo))
    {
        return false;
    }

    std::vector<USHORT> indices(indices32.size());
    for(size_t i = 0; i < indices32.size(); ++i)
        indices[i] = static_cast<uint16_t>(indices32[i]);
...
void SkinnedMeshApp::BuildCbvSrvUavDescriptorHeap()
{
    CbvSrvUavHeap& cbvSrvUavHeap = CbvSrvUavHeap::Get();
...
void SkinnedMeshApp::AddRenderItem(RenderLayer layer, const XMFLOAT4X4& world, const XMFLOAT4X4& texTransform, Material* mat, MeshGeometry* geo, SubmeshGeometry& drawArgs)
{
