    ...
    PSTR cmdLine, int showCmd)
{
    ...
    try
    {
        TexturedShapesApp theApp(hInstance);
...
TexturedShapesApp::TexturedShapesApp(HINSTANCE hInstance)
    : D3DApp(hInstance)
{

}

TexturedShapesApp::~TexturedShapesApp()
{
    if(md3dDevice != nullptr)
        FlushCommandQueue();
}

bool TexturedShapesApp::Initialize()
{
...
void TexturedShapesApp::CreateRtvAndDsvDescriptorHeaps()
{
...
void TexturedShapesApp::OnResize()
{
    D3DApp::OnResize();
...
void TexturedShapesApp::Update(const GameTimer& gt)
{
    OnKeyboardInput(gt);
    UpdateCamera(gt);
...
void TexturedShapesApp::Draw(const GameTimer& gt)
{
...
void TexturedShapesApp::UpdateImgui(const GameTimer& gt)
{
...
void TexturedShapesApp::OnMouseDown(WPARAM btnState, int x, int y)
{
...
void TexturedShapesApp::OnMouseUp(WPARAM btnState, int x, int y)
{
    ImGuiIO& io = ImGui::GetIO();
...
void TexturedShapesApp::OnMouseMove(WPARAM btnState, int x, int y)
{
    ImGuiIO& io = ImGui::GetIO();
...
void TexturedShapesApp::OnKeyboardInput(const GameTimer& gt)
{
}

void TexturedShapesApp::AnimateMaterials(const GameTimer& gt)
{
}

void TexturedShapesApp::UpdateCamera(const GameTimer& gt)
{
...
void TexturedShapesApp::UpdatePerObjectCB(const GameTimer& gt)
{
...
void TexturedShapesApp::UpdateMaterialBuffer(const GameTimer& gt)
{
...
void TexturedShapesApp::UpdateMainPassCB(const GameTimer& gt)
{
...
void TexturedShapesApp::LoadTextures()
{
    TextureLib& texLib = TextureLib::GetLib();
    texLib.Init(md3dDevice.Get(), *mUploadBatch.get());
}

void TexturedShapesApp::BuildCbvSrvUavDescriptorHeap()
{
    CbvSrvUavHeap& cbvSrvUavHeap = CbvSrvUavHeap::Get();
...
void TexturedShapesApp::BuildRootSignature()
{
...
void TexturedShapesApp::BuildShadersAndInputLayout()
{
#if defined(DEBUG) || defined(_DEBUG)  
...
void TexturedShapesApp::BuildPSOs()
{
...
void TexturedShapesApp::BuildFrameResources()
{
    MaterialLib& matLib = MaterialLib::GetLib();
...
void TexturedShapesApp::BuildMaterials()
{
    MaterialLib::GetLib().Init(md3dDevice.Get());
}

void TexturedShapesApp::AddRenderItem(RenderLayer layer, const DirectX::XMFLOAT4X4& world, const XMFLOAT4X4& texTransform, Material* mat, MeshGeometry* geo, SubmeshGeometry& drawArgs)
{
...
void TexturedShapesApp::BuildRenderItems()
{
    MaterialLib& matLib = MaterialLib::GetLib();
    ...
    XMStoreFloat4x4(&texTransform, XMMatrixScaling(1.0f, 0.5f, 1.0f));
    ...
    for(int i = 0; i < 5; ++i)
    {
        XMStoreFloat4x4(&texTransform, XMMatrixScaling(1.5f, 2.0f, 1.0f));
...
void TexturedShapesApp::DrawRenderItems(ID3D12GraphicsCommandList* cmdList, const std::vector<RenderItem*>& ritems)
{
    CbvSrvUavHeap& cbvSrvUavHeap = CbvSrvUavHeap::Get();
