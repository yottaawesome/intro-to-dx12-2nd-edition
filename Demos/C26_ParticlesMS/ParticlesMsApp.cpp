    ...
    PSTR cmdLine, int showCmd)
{
    ...
    try
    {
        ParticlesMsApp theApp(hInstance);
...
ParticlesMsApp::ParticlesMsApp(HINSTANCE hInstance)
    : D3DApp(hInstance)
{
...
ParticlesMsApp::~ParticlesMsApp()
{
    if(md3dDevice != nullptr)
        FlushCommandQueue();
}

bool ParticlesMsApp::Initialize()
{
...
void ParticlesMsApp::CreateRtvAndDsvDescriptorHeaps()
{
...
void ParticlesMsApp::Update(const GameTimer& gt)
{
    OnKeyboardInput(gt);
    ...
    mHelixParticleConstants.gHelixColorTint = XMFLOAT4(0.5f, 0.5f, 0.5f, 0.3f);

    mHelixParticleConstants.gHelixOrigin = XMFLOAT3(0.0f, 0.0f, 0.0f);
    ...
    const float numOscillations = 4.0f;
...
void ParticlesMsApp::Draw(const GameTimer& gt)
{
...
void ParticlesMsApp::UpdateImgui(const GameTimer& gt)
{
...
void ParticlesMsApp::OnMouseDown(WPARAM btnState, int x, int y)
{
...
void ParticlesMsApp::OnMouseUp(WPARAM btnState, int x, int y)
{
    ImGuiIO& io = ImGui::GetIO();
...
void ParticlesMsApp::OnMouseMove(WPARAM btnState, int x, int y)
{
    ImGuiIO& io = ImGui::GetIO();
...
void ParticlesMsApp::OnKeyboardInput(const GameTimer& gt)
{
...
void ParticlesMsApp::AnimateMaterials(const GameTimer& gt)
{
	
}

void ParticlesMsApp::UpdatePerObjectCB(const GameTimer& gt)
{
...
void ParticlesMsApp::UpdateMaterialBuffer(const GameTimer& gt)
{
...
void ParticlesMsApp::UpdateShadowTransform(const GameTimer& gt)
{
...
void ParticlesMsApp::UpdateMainPassCB(const GameTimer& gt)
{
...
void ParticlesMsApp::UpdateShadowPassCB(const GameTimer& gt)
{
...
void ParticlesMsApp::LoadTextures()
{
    TextureLib& texLib = TextureLib::GetLib();
    texLib.Init(md3dDevice.Get(), *mUploadBatch.get());
}

void ParticlesMsApp::LoadGeometry()
{
...
void ParticlesMsApp::BuildRootSignature()
{
    ...
    CD3DX12_ROOT_SIGNATURE_DESC gfxRootSigDesc(GFX_ROOT_ARG_COUNT, gfxRootParameters,
...
void ParticlesMsApp::BuildCbvSrvUavDescriptorHeap()
{
    CbvSrvUavHeap& cbvSrvUavHeap = CbvSrvUavHeap::Get();
...
void ParticlesMsApp::BuildShadersAndInputLayout()
{
...
void ParticlesMsApp::BuildFrameResources()
{
...
void ParticlesMsApp::BuildMaterials()
{
    MaterialLib::GetLib().Init(md3dDevice.Get());
}

void ParticlesMsApp::AddRenderItem(RenderLayer layer, const XMFLOAT4X4& world, const XMFLOAT4X4& texTransform, Material* mat, MeshGeometry* geo, SubmeshGeometry& drawArgs)
{
...
void ParticlesMsApp::BuildRenderItems()
{
    MaterialLib& matLib = MaterialLib::GetLib();
...
{
    GraphicsResource cbHandle = mLinearAllocator->AllocateConstant(mHelixParticleConstants);
...
void ParticlesMsApp::DrawRenderItems(ID3D12GraphicsCommandList* cmdList, const std::vector<RenderItem*>& ritems)
{
...
void ParticlesMsApp::DrawSceneToShadowMap()
{
    PsoLib& psoLib = PsoLib::GetLib();
...
void ParticlesMsApp::DrawNormalsAndDepth()
{
    PsoLib& psoLib = PsoLib::GetLib();
    ...
    auto transition = CD3DX12_RESOURCE_BARRIER::Transition(normalMap,
