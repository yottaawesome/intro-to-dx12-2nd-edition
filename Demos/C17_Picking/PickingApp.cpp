...
void PickingApp::UpdateImgui(const GameTimer& gt)
{
...
void PickingApp::UpdatePerObjectCB(const GameTimer& gt)
{
...
void PickingApp::BuildCbvSrvUavDescriptorHeap()
{
    CbvSrvUavHeap& cbvSrvUavHeap = CbvSrvUavHeap::Get();
...
void PickingApp::AddRenderItem(RenderLayer layer, const XMFLOAT4X4& world, const XMFLOAT4X4& texTransform, Material* mat, MeshGeometry* geo, SubmeshGeometry& drawArgs)
{
...
void PickingApp::Pick(int sx, int sy)
{
    ...
    auto detView = XMMatrixDeterminant(V);
    ...
    for(auto ri : mRitemLayer[(int)RenderLayer::Opaque])
    {
        ...
        XMVECTOR rayOriginL = XMVector3TransformCoord(rayOriginV, toLocal);
        XMVECTOR rayDirL = XMVector3TransformNormal(rayDirV, toLocal);
        ...
        float boxDist = 0.0f;
        if(ri->Bounds.Intersects(rayOriginL, rayDirL, boxDist))
        {
            ...
            for(UINT i = 0; i < triCount; ++i)
            {
                ...
                if(TriangleTests::Intersects(rayOriginL, rayDirL, v0, v1, v2, t))
                {
                    ...
                    // the current mesh.
                    float t_world = XMVectorGetX(XMVector3Length(hitPosW - mCamera.GetPosition()));
