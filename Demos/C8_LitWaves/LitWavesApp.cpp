...
void LitWavesApp::CreateRtvAndDsvDescriptorHeaps()
{
...
void LitWavesApp::UpdateImgui(const GameTimer& gt)
{
...
void LitWavesApp::UpdatePerObjectCB(const GameTimer& gt)
{
...
void LitWavesApp::UpdateWaves(const GameTimer& gt)
{
    ...
    for(int i = 0; i < mWaves->VertexCount(); ++i)
    {
        ...
        verts[i].TexC = XMFLOAT2(0.0f, 0.0f);
...
void LitWavesApp::BuildCbvSrvUavDescriptorHeap()
{
    CbvSrvUavHeap& cbvSrvUavHeap = CbvSrvUavHeap::Get();
...
void LitWavesApp::BuildFrameResources()
{
    ...
    for(int i = 0; i < gNumFrameResources; ++i)
    {
        mFrameResources.push_back(
            ...
            passCount, mMaterials.size(), mWaves->VertexCount()));
    }
}
...
void LitWavesApp::BuildMaterials()
{
    ...
    AddMaterial("lakeBlue",
                XMFLOAT4(0.2f, 0.2f, 0.8f, 1.0f),
                XMFLOAT3(0.05f, 0.05f, 0.05f),
                0.3f);
}

void LitWavesApp::AddRenderItem(RenderLayer layer, const DirectX::XMFLOAT4X4& world, Material* mat, MeshGeometry* geo, SubmeshGeometry& drawArgs)
{
...
std::unique_ptr<MeshGeometry> LitWavesApp::BuildLandGeometry(
    ID3D12Device* device,
...
std::unique_ptr<MeshGeometry> LitWavesApp::BuildWaveGeometry(
    ID3D12Device* device,
    ResourceUploadBatch& uploadBatch)
{
