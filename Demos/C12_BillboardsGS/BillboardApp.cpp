...
BillboardApp::BillboardApp(HINSTANCE hInstance)
    : D3DApp(hInstance)
{
...
bool BillboardApp::Initialize()
{
    ...
    std::unique_ptr<MeshGeometry> treeSpriteGeo = BuildBillboardSpriteGeometry(
        md3dDevice.Get(), *mUploadBatch.get());
    if(treeSpriteGeo != nullptr)
    {
...
void BillboardApp::CreateRtvAndDsvDescriptorHeaps()
{
...
void BillboardApp::Update(const GameTimer& gt)
{
    OnKeyboardInput(gt);
    UpdateCamera(gt);
...
void BillboardApp::Draw(const GameTimer& gt)
{
...
void BillboardApp::UpdateImgui(const GameTimer& gt)
{
...
void BillboardApp::OnMouseDown(WPARAM btnState, int x, int y)
{
...
void BillboardApp::OnMouseUp(WPARAM btnState, int x, int y)
{
    ImGuiIO& io = ImGui::GetIO();
...
void BillboardApp::OnMouseMove(WPARAM btnState, int x, int y)
{
    ImGuiIO& io = ImGui::GetIO();
...
void BillboardApp::OnKeyboardInput(const GameTimer& gt)
{
}

void BillboardApp::AnimateMaterials(const GameTimer& gt)
{
...
void BillboardApp::UpdateCamera(const GameTimer& gt)
{
...
void BillboardApp::UpdatePerObjectCB(const GameTimer& gt)
{
...
void BillboardApp::UpdateMaterialBuffer(const GameTimer& gt)
{
    MaterialLib& matLib = MaterialLib::GetLib();
...
void BillboardApp::UpdateMainPassCB(const GameTimer& gt)
{
...
void BillboardApp::UpdateWaves(const GameTimer& gt)
{
...
void BillboardApp::BuildRootSignature()
{
...
void BillboardApp::BuildShadersAndInputLayout()
{
#if defined(DEBUG) || defined(_DEBUG)  
...
#define COMMA_DEBUG_ARGS
    ...
    std::vector<LPCWSTR> gsArgs = std::vector<LPCWSTR> { L"-E", L"GS", L"-T", L"gs_6_6" COMMA_DEBUG_ARGS };
...
void BillboardApp::BuildPSOs()
{
    ...
    ThrowIfFailed(md3dDevice->CreateGraphicsPipelineState(&treeSpriteWireframePsoDesc, IID_PPV_ARGS(&mPSOs["treeSprites_wireframe"])));
}

void BillboardApp::BuildFrameResources()
{
    MaterialLib& matLib = MaterialLib::GetLib();
...
void BillboardApp::BuildMaterials()
{
    MaterialLib::GetLib().Init(md3dDevice.Get());
}

void BillboardApp::AddRenderItem(
    RenderLayer layer, 
...
void BillboardApp::BuildRenderItems()
{
    MaterialLib& matLib = MaterialLib::GetLib();
...
void BillboardApp::DrawRenderItems(ID3D12GraphicsCommandList* cmdList, const std::vector<RenderItem*>& ritems)
{
    CbvSrvUavHeap& cbvSrvUavHeap = CbvSrvUavHeap::Get();
...
std::unique_ptr<MeshGeometry> BillboardApp::BuildLandGeometry(
    ID3D12Device* device,
...
std::unique_ptr<MeshGeometry> BillboardApp::BuildWaveGeometry(
    ID3D12Device* device,
    ResourceUploadBatch& uploadBatch)
{
...
std::unique_ptr<MeshGeometry> BillboardApp::BuildBillboardSpriteGeometry(ID3D12Device* device, DirectX::ResourceUploadBatch& uploadBatch)
{
    mTreeSprites.clear();

    constexpr int gridY = 11;
    constexpr int gridX = 11;
    ...
    for(int i = 0; i < gridY; ++i)
    {
        for(int j = 0; j < gridX; ++j)
        {
            ...
            float jitterX = MathHelper::RandF(-5.0f, 5.0f);
            float jitterZ = MathHelper::RandF(-5.0f, 5.0f);

            float x = -50.0f + i * 10.0f;
            float z = +50.0f - j * 10.0f;
            ...
            float size = MathHelper::RandF(12.0f, 16.0f);

            TreeSpriteVertex sprite;
            sprite.Pos = XMFLOAT3(x, y, z);
    ...
    for(size_t i = 0; i < mTreeSprites.size(); ++i)
        indices[i] = i;

    const UINT vbByteSize = (UINT)mTreeSprites.size() * sizeof(TreeSpriteVertex);
    ...
    geo->VertexBufferCPU.resize(vbByteSize);
    CopyMemory(geo->VertexBufferCPU.data(), mTreeSprites.data(), vbByteSize);
    ...
    submesh.StartIndexLocation = 0;
    submesh.BaseVertexLocation = 0;
    submesh.VertexCount = mTreeSprites.size();
...
float BillboardApp::GetHillsHeight(float x, float z)const
{
    return 0.3f*(z*sinf(0.1f*x) + x*cosf(0.1f*z));
}

DirectX::XMFLOAT3 BillboardApp::GetHillsNormal(float x, float z)const
{
    // n = (-df/dx, 1, -df/dz)
    XMFLOAT3 n(
