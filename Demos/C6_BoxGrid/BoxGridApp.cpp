...
BoxGridApp::BoxGridApp(HINSTANCE hInstance)
    : D3DApp(hInstance)
{

}

BoxGridApp::~BoxGridApp()
{
    if(md3dDevice != nullptr)
        FlushCommandQueue();
}

bool BoxGridApp::Initialize()
{
    ...
    for(UINT i = 0; i < BOX_GRID_SIZE; ++i)
    {
        for(UINT j = 0; j < BOX_GRID_SIZE; ++j)
        {
            ...
            XMMATRIX W = XMMatrixTranslation(x, 0.0f, z);
            XMStoreFloat4x4(&mWorld[i*BOX_GRID_SIZE+j], W);
        }
    }
...
void BoxGridApp::CreateRtvAndDsvDescriptorHeaps()
{
...
void BoxGridApp::Update(const GameTimer& gt)
{
...
void BoxGridApp::Draw(const GameTimer& gt)
{
...
void BoxGridApp::UpdateImgui(const GameTimer& gt)
{
...
void BoxGridApp::OnMouseDown(WPARAM btnState, int x, int y)
{
...
void BoxGridApp::OnMouseUp(WPARAM btnState, int x, int y)
{
    ImGuiIO& io = ImGui::GetIO();
...
void BoxGridApp::OnMouseMove(WPARAM btnState, int x, int y)
{
    ImGuiIO& io = ImGui::GetIO();
...
void BoxGridApp::BuildCbvSrvUavDescriptorHeap()
{
    CbvSrvUavHeap& cbvSrvUavHeap = CbvSrvUavHeap::Get();
...
void BoxGridApp::BuildConstantBuffers()
{
    CbvSrvUavHeap& cbvSrvUavHeap = CbvSrvUavHeap::Get();
...
void BoxGridApp::BuildRootSignature()
{
...
void BoxGridApp::BuildShadersAndInputLayout()
{
#if defined(DEBUG) || defined(_DEBUG)  
...
void BoxGridApp::BuildBoxGeometry(ID3D12Device* device, DirectX::ResourceUploadBatch& uploadBatch)
{
